...
function HomeClient({ email }: { email: string }) {
  ...
  const features = [
    ...
    },
    {
      ...
      desc: "You Controlled exactly what the AI knows and answers."
    },
    {
      title: "Always Online",
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          {email ? (
            <div className="relative" ref={popupRef}>
              <button
                className="w-10 h-10 rounded-full bg-black text-white flex cursor-pointer
                        items-center justify-center font-semibold hover:scale-105 transition"
              ...
              <AnimatePresence>
                    ...
                    exit={{opacity:0,y:-6}}
                    className="absolute right-0 mt-3 w-44 bg-white rounded-xl shadow-xl border border-zinc-200
              overflow-hidden">
            ...
            <motion.button
              className="px-5 py-2 rounded-full bg-black text-white text-sm font-medium hover:bg-zinc-800 transition
                disabled:opacity-60 flex items-center gap-2 cursor-pointer"
              onClick={handleLogin}
              disabled={loading}
            >
            ...
            >
                ...
                <div className="mt-10 flex gap-4">
                  {email ? <button className="px-7 py-3 rounded-xl bg-black text-white font-medium
                    ...
                    onClick={()=>navigate.push("/dashboard")}>Go To Dashboard</button>
                  :<button className="px-7 py-3 rounded-xl bg-black text-white font-medium
                    hover:bg-zinc-800 transition disabled:opacity-60 cursor-pointer">Get Started</button>}
                
                <a href="#feature" className="px-7 py-3 rounded-xl border border-zinc-700 hover:bg-zinc-100
                transition cursor-pointer">Learn More</a>
                </div>
            ...
            >
              <div className="rounded-2xl bg-white shadow-2xl border border-zinc-200 p-6">
                ...
                <div className="space-y-3">
                  ...
                  ml-auto w-fit">Do You Offer cash on delivery</div>
